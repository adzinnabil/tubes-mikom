...
def detect_color(frame, colors):
    ...
    """
    detected_color = None
    min_area = 100  # Luas area minimum
    ...
    for color_name, (low, high) in colors.items():
        ...
        if contours:
            ...
            if area > min_area:  # Hanya proses jika luas area lebih dari 100
...
def main():
    global last_detected_time, last_color
    ...
    while True:
        ...
        if detected_color:
            ...
            if detected_color == last_color:
                if current_time - last_detected_time >= 1:  # 1 detik
                    print(f"Warna terdeteksi selama 1 detik: {detected_color}")
